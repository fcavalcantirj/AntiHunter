...
String getDiagnostics() {
    ...
    s += "Mesh Dedup: " + String(meshDedupCount()) + " MACs (" + (getMeshSessionDedup() ? "session" : "ttl") + ")\n";
    ...
    if (rtcAvailable) {
        if (rtcSynced) {
            ...
            // RTC has valid time from battery backup even without GPS sync
    ...
    if (droneDetectionEnabled) {
        ...
        { std::lock_guard<std::mutex> dronesLock(detectedDronesMutex); uniqueDronesCount = (int)detectedDrones.size(); }
